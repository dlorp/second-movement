...
static void _emergence_face_update_display(emergence_face_state_t *state) {
    ...
    switch (state->display_mode) {
        ...
        case 2: {  // Stats: "E Lv2 3d"
            word_level_t level = zone_words_get_level(metrics.em);
            snprintf(buf, sizeof(buf), "E Lv%d %dd", level, state->streak_days);
...
void emergence_face_activate(void *context) {
    ...
    uint16_t day_of_year = date_time.unit.day;  // Simplified day tracking
    ...
    if (state->last_check_day != day_of_year) {
        ...
        // Simple consecutive check (doesn't handle year rollover, but good enough)

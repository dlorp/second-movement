...
// ============================================================================
// 5. Phase 4F: WK Activity Baseline (7-Day Rolling Average)
// ============================================================================

typedef struct {
    uint16_t daily_totals[7];    // Movement counts per day, 7-day circular buffer (14 bytes)
    uint8_t day_index;           // Current day position in buffer (0-6)
    uint8_t baseline_per_hour;   // Computed baseline: avg(daily_totals) / 16 active hours
} wk_baseline_t;  // 16 bytes
...
typedef struct {
    ...
    wk_baseline_t wk_baseline;   // Phase 4F: Activity baseline tracking
    bool initialized;
} sleep_telemetry_state_t;  // ~487 bytes total
...
/**
 * Phase 4F: Update WK baseline with today's total movement.
 * Called once per day at midnight to advance circular buffer and recompute baseline.
 * 
 * @param wk WK baseline state
 * @param daily_movement Total movement count for the day
 */
void wk_baseline_update_daily(wk_baseline_t *wk, uint16_t daily_movement);

/**
 * Phase 4F: Get current activity baseline (movements per hour).
 * 
 * @param wk WK baseline state
...
/**
 * Phase 4F: Derive "active" threshold from baseline.
 * Active threshold = 1.5× baseline (integer: baseline + baseline/2)
 * 
 * @param wk WK baseline state
 * @return Active threshold (movements per hour)
 */
uint8_t wk_baseline_get_active_threshold(const wk_baseline_t *wk);

/**
 * Phase 4F: Derive "very active" threshold from baseline.
 * Very active threshold = 2.5× baseline (integer: baseline*2 + baseline/2)
 * 
 * @param wk WK baseline state
 * @return Very active threshold (movements per hour)
 */
